...
class SpiderSpider(scrapy.Spider):
    ...
    start_urls = ['http://books.toscrape.com/catalogue/category/books/fantasy_19/']
    # New 'base_url' variable
    base_url = 'http://books.toscrape.com'
    start = 'http://books.toscrape.com/catalogue/category/books/fantasy_19/'

    def parse(self, response):
        all_books = response.xpath('//li[@class="col-xs-6 col-sm-4 col-md-3 col-lg-3"]/article')
        
        for book in all_books:
            book_url = self.start_urls[0].replace('category/books/fantasy_19/', '')
